...
oBrowser = function (name) {
	...
	this.settings_context_menu = function (x, y) { // poo
	    ...
	    menu.newCheckMenu(src_menu, 'Library', 'Playlist', () => !ppt.sourceMode ? 0 : 1);
	    menu.newEntry({menuName: src_menu, entryText: 'Library', func: () => {ppt.sourceMode = 0; window.SetProperty("_PROPERTY: Source Mode", ppt.sourceMode); window.Reload(); }, flags: () => ppt.sourceMode === 0 ? MF_CHECKED : MF_STRING});
	    ...
	    const column_menu = menu.newMenu('Columns');
	    menu.newEntry({menuName: column_menu, entryText: 'Album', func: () => {ppt.tagMode = 1; window.SetProperty("_PROPERTY: Tag Mode", ppt.tagMode); g_image_cache = new image_cache; brw.populate(true);}, flags: () => ppt.tagMode === 1 ? MF_CHECKED : MF_STRING});
	    menu.newEntry({menuName: column_menu, entryText: 'Artist', func: () => {ppt.tagMode = 2; window.SetProperty("_PROPERTY: Tag Mode", ppt.tagMode); g_image_cache = new image_cache; brw.populate(true);}, flags: () => ppt.tagMode === 2 ? MF_CHECKED : MF_STRING});
	    menu.newEntry({menuName: column_menu, entryText: 'Genre', func: () => {ppt.tagMode = 3; window.SetProperty("_PROPERTY: Tag Mode", ppt.tagMode); g_image_cache = new image_cache; brw.populate(true);}, flags: () => ppt.tagMode === 3 ? MF_CHECKED : MF_STRING});

	    const display_menu = menu.newMenu('Display');
	    menu.newEntry({menuName: display_menu, entryText: 'Column', func: () => {ppt.panelMode = 0; window.SetProperty("_PROPERTY: Display Mode", ppt.panelMode); g_image_cache = new image_cache; get_metrics(); brw.setList(); brw.update();}, flags: () => ppt.panelMode === 0 ? MF_CHECKED : MF_STRING});
	    menu.newEntry({menuName: display_menu, entryText: 'Album Art (bottom labels)', func: () => {ppt.panelMode = 1; window.SetProperty("_PROPERTY: Display Mode", ppt.panelMode); g_image_cache = new image_cache; get_metrics(); brw.setList(); brw.update();}, flags: () => ppt.panelMode === 1 ? MF_CHECKED : MF_STRING});
	    menu.newEntry({menuName: display_menu, entryText: 'Album Art (right labels)', func: () => {ppt.panelMode = 2; window.SetProperty("_PROPERTY: Display Mode", ppt.panelMode); g_image_cache = new image_cache; get_metrics(); brw.setList(); brw.update();}, flags: () => ppt.panelMode === 2 ? MF_CHECKED : MF_STRING});
	    menu.newEntry({menuName: display_menu, entryText: 'Album Art Grid', func: () => {ppt.panelMode = 3; window.SetProperty("_PROPERTY: Display Mode", ppt.panelMode); g_image_cache = new image_cache; get_metrics(); brw.setList(); brw.update();}, flags: () => ppt.panelMode === 3 ? MF_CHECKED : MF_STRING});
	    menu.newEntry({menuName: display_menu, entryText: 'sep'});
	    menu.newEntry({menuName: display_menu, entryText: 'Auto-fill', func: () => {ppt.autoFill = !ppt.autoFill; window.SetProperty("_DISPLAY: Auto-fill", ppt.autoFill); window.Reload();}, flags: () => ppt.autoFill ? MF_CHECKED : MF_STRING});
	    menu.newEntry({menuName: display_menu, entryText: 'sep'});
	    menu.newEntry({menuName: display_menu, entryText: 'Header Bar', func: () => {ppt.showHeaderBar = !ppt.showHeaderBar; window.SetProperty("_DISPLAY: Show Top Bar", ppt.showHeaderBar); get_metrics();}, flags: () => ppt.showHeaderBar ? MF_CHECKED : MF_STRING});
	    menu.newEntry({menuName: display_menu, entryText: 'Aggregate Item', func: () => {ppt.showAllItem = !ppt.showAllItem; window.SetProperty("_PROPERTY: Show ALL item", ppt.showAllItem); brw.populate(false);}, flags: () => ppt.showAllItem ? MF_CHECKED : MF_STRING});
	    menu.newEntry({menuName: display_menu, entryText: 'sep'});
	    menu.newEntry({menuName: display_menu, entryText: 'Enable Disk Image Cache', func: () => {ppt.enableDiskCache = !ppt.enableDiskCache; window.SetProperty("SYSTEM Disk Cache", ppt.enableDiskCache); window.Reload();}, flags: () => ppt.enableDiskCache ? MF_CHECKED : MF_STRING});
        menu.newEntry({entryText: 'sep'});
...
function on_playback_new_track(metadb) {
	g_metadb = metadb;
	g_wallpaperImg = setWallpaperImg();
	if (ppt.col_mode === 2) on_colours_changed();
	brw.repaint();
};
