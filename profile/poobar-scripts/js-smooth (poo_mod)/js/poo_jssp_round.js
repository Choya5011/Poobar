...
ppt = {
	...
	// poo
	round: window.GetProperty('_DISPLAY: Round corners', false),
...
cScrollBar = {
	...
	visible: true,
	themed: false,
	defaultWidth: !ppt.round ? get_system_scrollbar_width() : scale(10), // poo
	width: !ppt.round ? get_system_scrollbar_width() : scale(10), // poo
	ButtonType: {
		cursor: 0,
		up: 1,
		down: 2
		...
		if (this.offset > 0) {
			...
			for (var i = start_; i < end_; i++) {
				...
				if (!ppt.round) { // poo
				    ...
				    gr.FillRoundRect(cx + bg_margin_left, cy + bg_margin_top, cw - bg_margin_left * 2 - this.scr_w, ch - bg_margin_top * 2, poo.arc.d, poo.arc.d, RGBA(0, 0, 0, 130));
				    gr.DrawRoundRect(cx + bg_margin_left, cy + bg_margin_top, cw - bg_margin_left * 2 - this.scr_w - 1, ch - bg_margin_top * 2 - 1, poo.arc.d, poo.arc.d, 1.0, RGBA(255, 255, 255, 20));
				}
			...
			if (cPlaylistManager.blink_counter > -1 && cPlaylistManager.blink_row > 0) {
				...
				if (!ppt.round) { // poo
				    ...
				    gr.DrawRoundRect(cx + bg_margin_left + 1, cy_ + bg_margin_top + 1, cw - bg_margin_left * 2 - this.scr_w - 2, ch - bg_margin_top * 2 - 2, poo.arc.d, poo.arc.d, 2.0, RGBA(255, 255, 255, 240));
				}
			} else {
			...
			if (this.scr_w > 0) {
                ...
                if (!ppt.round) { // poo
                    ...
                    gr.FillRoundRect(cx + cw - this.scr_w, this.scr_cursor_y, this.scr_w - 4, this.scr_cursor_h, poo.arc.d, poo.arc.d, RGBA(255, 255, 255, 100));
                }
			};
...
oScrollbar = function (themed) {
	...
	this.setCursorButton = function () {
		...
		if (ppt.round) { gb.SetSmoothingMode(2); } else { gb.SetSmoothingMode(0); } // poo
		// Draw Themed Scrollbar (lg/col)
		if (this.themed) {
			try {
				...
				if (!ppt.round) { // poo
				    ...
				    gb.FillRoundRect(1, 0, this.cursorw - 2, this.cursorh, poo.arc.s, poo.arc.s, blendColors(this.color_txt, this.color_bg, 0.5));
				}
			};
		} else {
		    if (!ppt.round) { // poo
                ...
                gb.FillRoundRect(1, 0, this.cursorw - 2, this.cursorh, poo.arc.s, poo.arc.s, blendColors(this.color_txt, this.color_bg, 0.5) & 0x88ffffff);
                gb.DrawRoundRect(1, 0, this.cursorw - 2 - 1, this.cursorh - 1, poo.arc.s, poo.arc.s, 1.0, this.color_txt & 0x44ffffff);
		    }
		};
		...
		if (this.themed) {
			try {
			    ...
			    if (!ppt.round) { // poo
                    ...
                    gb.FillRoundRect(1, 0, this.cursorw - 2, this.cursorh, poo.arc.s, poo.arc.s, blendColors(this.color_txt, this.color_bg, 0.3));
                }
			};
		} else {
		    if (!ppt.round) { // poo
                ...
                gb.FillRoundRect(1, 0, this.cursorw - 2, this.cursorh, poo.arc.s, poo.arc.s, blendColors(this.color_txt, this.color_bg, 0.3) & 0x88ffffff);
                gb.DrawRoundRect(1, 0, this.cursorw - 2 - 1, this.cursorh - 1, poo.arc.s, poo.arc.s, 1.0, this.color_txt & 0x44ffffff);
            }
		};
		...
		if (this.themed) {
			try {
			    ...
			    if (!ppt.round) { // poo
                    ...
                    gb.FillRoundRect(1, 0, this.cursorw - 2, this.cursorh, poo.arc.s, poo.arc.s, blendColors(this.color_txt, this.color_bg, 0.05));
                }
			};
		} else {
		    if (!ppt.round) { // poo
                ...
                gb.FillRoundRect(1, 0, this.cursorw - 2, this.cursorh, poo.arc.s, poo.arc.s, blendColors(this.color_txt, this.color_bg, 0.05) & 0x88ffffff);
                gb.DrawRoundRect(1, 0, this.cursorw - 2 - 1, this.cursorh - 1, poo.arc.s, poo.arc.s, 1.0, this.color_txt & 0x44ffffff);
            }
		};
						...
						if (ay > 0 - (ghrh * ah) && ay < this.h + (ghrh * ah)) {
							...
							if (g_selected) {
								...
								if (!ppt.round) { // poo
								    ...
								    gr.FillRoundRect(ax, ay - ((ghrh - 1) * ah), aw, ah * ghrh - 1, poo.arc.h, poo.arc.h, g_color_selected_bg & 0xb0ffffff);
								}
							} else {
								...
								if (!ppt.round) {
                                    ...
                                    gr.FillRoundRect(ax, ay - ((ghrh - 1) * ah), aw, ah * ghrh - 1, poo.arc.h, poo.arc.h, g_color_normal_txt & 0x05ffffff);
                                    gr.FillRoundRect(ax, ay - ((ghrh - 1) * ah), aw, 1, poo.arc.h, poo.arc.h, g_color_normal_txt & 0x08ffffff);
								}
							};
							// ==========
							// cover art
							...
							if (ghrh > 1 && cover.show) {
								...
								if (this.groups[g].cover_img != null) {
									...
									if (!ppt.round) { // poo
									    ...
									    gr.SetSmoothingMode(2);
									    const aa = apply_corners_mask(this.groups[g].cover_img, 0.1)
									    gr.DrawImage(aa, cv_x, cv_y, cv_w + 1, cv_h + 1, 1, 1, cv_w, cv_h, 0, 255);
									    gr.DrawRoundRect(cv_x, cv_y, cv_w - 1, cv_h - 1, 8, 8, 2.0, g_color_normal_bg);
									    gr.DrawRoundRect(cv_x, cv_y, cv_w - 1, cv_h - 1, 8, 8, 1.0, g_color_normal_txt & 0x25ffffff);
									}
								} else {
								...
								if (ppt.doubleRowText) {
									...
									if (this.nowplaying && this.rows[i].playlistTrackId == this.nowplaying.PlaylistItemIndex) { // now playing track
                                        ...
                                        if (g_seconds == 0 || g_seconds / 2 == Math.floor(g_seconds / 2)) {
                                            ...
                                            gr.DrawString(chars.play, g_font_fluent_small, g_color_normal_txt, ax + ppt.rowHeight * -0.14, ay + ppt.rowHeight * 0.285, ah, ah, DT_CENTER | DT_VCENTER | DT_NOPREFIX);
                                        }
                                        ...
                                        */
                                        ...
                                        for (let q = 0; q < queue.length; q++) {
                                            ...
                                            if (
                                                queue[q].PlaylistIndex === g_active_playlist &&
                                                queue[q].PlaylistItemIndex === this.rows[i].playlistTrackId
                                            ) {
                                                queue_num = q + 1;
                                                break;
                                            }
                                        ...
                                        if (queue_num !== null) {
                                            // axes
                                            // 35 = 38, rounded elsewhere in the script
                                            let queueIconX = (ppt.rowHeight > 38) ? ax + ppt.rowHeight * -0.1 : ax + ppt.rowHeight * -0.01;
                                            //console.log(`rowHeight: ${ppt.rowHeight}, queueIconX: ${queueIconX}`); // debug
                                            let queueIconY = ay + ppt.rowHeight * 0.17;
                                            let queueTextX = (ppt.rowHeight > 38) ? ax + ppt.rowHeight * -0.2 : ax + ppt.rowHeight * -0.12;
                                            let queueTextY = (ppt.rowHeight > 38) ? ay + ppt.rowHeight * 0.001 : ay + ppt.rowHeight * 0.001;

                                            // Comment: ah = ppt.rowHeight
                                            gr.DrawString(chars.stop, g_font_fluent_queue, g_color_normal_txt, queueIconX, queueIconY, ah, ah, DT_CENTER | DT_VCENTER | DT_NOPREFIX);
                                            let display_text = queue_num < 10 ? "0" + queue_num : "" + queue_num;
                                            gr.GdiDrawText(display_text, g_font_queue_idx, g_color_normal_txt, queueTextX, queueTextY, ah, ah, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
                                        } else {
